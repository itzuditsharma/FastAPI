...
def get_patient_details(patient_id : str = Path(..., description="Id of the patient")):
    data = load_data()
    if patient_id in data:
        return data[patient_id]
    raise HTTPException(status_code=404, detail="Patient is not found")

# http://127.0.0.1:8000/sorted_age?order_by=desc
@app.get("/sorted_weight")
def sort_based_on_weight(order_by : str = Query('asc', description="sort in asc or desc")):
    data = load_data()
    sort_order = True if order_by == 'desc' else False
    sorted_data = sorted(data.values(), key = lambda x : x.get('weight', 0), reverse=sort_order)
    return sorted_data
...
def delete_patient(patient_id: str):
    ...
    return JSONResponse(status_code=200, content=f"User with id: {patient_id} deleted")